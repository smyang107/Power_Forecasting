...
regressor = SVR(kernel='linear')#LinearSVC
#LinearSVC:‘linear’, ‘poly’, ‘rbf’, ‘sigmoid’, ‘precomputed’
# regressor= SVR(kernel='poly')
#regressor = SVR(kernel='rbf', C=1.25)
...
plt.savefig('./res/SVR.png')

# 保存模型
# pickle.dump(regressor, open("./res/RF_tianqi.dat", "wb"))
s = pickle.dumps(regressor)
with open("./res/SVR_tianqi.model",'wb+') as f:#注意此处mode是'wb+'，表示二进制写入
    f.write(s)
...
print("R2",r2)
Y3=pd.DataFrame(Y3)
Y3.to_csv('res/SVR_tianqi_3_month.csv')
...
# 训练
# f = open('./res/SVR_tianqi.model','rb') #注意此处model是rb
# s = f.read()
...
print("R2",r2)
Y3=pd.DataFrame(Y3)
Y3.to_csv('res/SVR_tianqi_10_day.csv')