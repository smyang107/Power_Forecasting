...
#
X = pd.read_csv('data/AllData1_2_X.csv') ### 加上气候信息、时间信息和新增的特征
X=X.iloc[:100, :].values
print(X.shape)#(1343, 5)numpy.ndarray
Y = pd.read_csv('data/data1_2_Y.csv')
Y=Y.iloc[:100, :].values
...
# features=Features.values[:119389,2:]
...
pre_X=pd.read_csv('data/predict1_2_X.csv').values[:100,:]
pre_Y=pd.read_csv('data/pre1_2_Y.csv').values[:100,:]
feature1=Features.values[119336:119336+100,2:]
pre_X=np.concatenate((pre_X,feature1),axis=1)
...
X1 = pd.read_csv('data/AllData1_1_X.csv') ### 加上气候信息、时间信息和新增的特征
X1=X1.iloc[:, :].values
Y1 = pd.read_csv('data/data1_1_Y .csv')

feature1=Features.values[X.shape[0]:X1.shape[0],2:]
X_train=np.concatenate((X1[X.shape[0]:,:],feature1),axis=1)
Y_train=Y1[Y.shape[0]:]
f = open('./res/RF_tianqi.model','rb') #注意此处model是rb
s = f.read()
model = pickle.loads(s)
model.fit(X_train,Y_train)

print('############ 10 天间隔 15 分钟的负荷预测结果:预测精度 ############')

pre_X=pd.read_csv('data/predict1_1_X.csv').values
pre_Y=pd.read_csv('data/pre1_1_Y.csv').values
Y3 = regressor.predict(pre_X)

MSE = np.linalg.norm(Y3-pre_X, ord=2)**2/len(pre_X)
print("MSE",MSE)#79.26742582898571

#RMSE--相当于y-y_hat的二阶范数/根号n
RMSE = np.linalg.norm(Y3-pre_X, ord=2)/len(pre_X)**0.5
print("RMSE",RMSE) #8.903225585650725

### MAE--相当于y-y_hat的一阶范数/n
MAE = np.linalg.norm(Y3-pre_X, ord=1)/len(pre_X)
print("MAE",MAE) #6.396413824234643

##
MAPE = np.mean(np.abs((Y3-pre_X) / pre_X)) * 100
