...
num_round = 2     # 使用线程数
#bst = xgb.train(param, dtrain, num_round)   # 训练(二分类)
...
plt.savefig('./res/xgboost.png')

# 保存模型
# pickle.dump(regressor, open("./res/RF_tianqi.dat", "wb"))
s = pickle.dumps(regressor)
with open("./res/xgboost_tianqi.model",'wb+') as f:#注意此处mode是'wb+'，表示二进制写入
    f.write(s)
...
print("R2",r2)
Y3=pd.DataFrame(Y3)
Y3.to_csv('res/xgboost_tianqi_3_month.csv')
...
# 训练
f = open('./res/xgboost_tianqi.model','rb') #注意此处model是rb
s = f.read()
...
Y3=pd.DataFrame(Y3)
Y3.to_csv('res/xgboost_tianqi_10_day.csv')